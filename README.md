Link Youtube https://youtu.be/YATuvDxKNw4
...
1) Fitur Utama (sesuai Core Requirements)

Distributed Lock Manager (Raft)

3 node, leader election, write hanya di leader (followers redirect)

Shared & Exclusive locks, deadlock detection sederhana (wait-for + timeout)

Network partition → hanya quorum yang boleh commit

Distributed Queue System

Consistent hashing untuk pemilikan shard (topic,key) → owners

Persistence (Redis) + recovery, multiple producers/consumers

At-least-once delivery (visibility TTL + ACK / ACK owner)

Distributed Cache Coherence

MESI (M/E/S/I) + invalidation antar node

LRU eviction, TTL per key

Metrics: hits, misses, puts, evictions, inv_sent/recv

Containerization

Dockerfile per komponen, docker-compose orchestration

.env untuk konfigurasi, scaling mudah (menambah replika)
...
A. Distributed Lock Manager (Raft) — 25 poin
